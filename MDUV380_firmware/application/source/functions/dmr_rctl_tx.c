	...
	if (dmr_rctl_stock_should_act(cmd, dst, trxDMRID, dmrRctlAllowMask()))
	{
		...
		switch (cmd)
		{
			case DMR_RCTL_STOCK_CHECK:
				 ...
				 * онлайн. Формат знято з ефіру (RCTL_COMPAT.md §5a).
				 *
				 * Відповідаємо НАВІТЬ ЗАБЛОКОВАНІ (stun): емпірика з ефіру (2026-09-12,
				 * RCTL_COMPAT.md §6) показала, що СТОКОВА відповідає на Check й у сні -- тож
				 * раніше тут було хибне припущення (мовчання «як стокова» було не як стокова).
				 * Гейт дозволів усе одно діє: відповідаємо лише тому, хто має право Check (should_act
			...
			case DMR_RCTL_STOCK_MONITOR:
				...
				/* Монітор: НЕ шлемо окрему CSBK-квитанцію. Захват BBD_0002 довів, що стокова-
				 * командир чекає САМЕ ГОЛОСОВИЙ виклик, і то ШВИДКО: у попередній збірці голос
				 * стартував аж через 840 мс (наш defer) -- стокова не дочекалась і видала помилку
				 * та повтор. Стокова як ЦІЛЬ теж відповідає на монітор просто голосовим викликом
				 * (RCTL_COMPAT.md §2.5). Тож ключуємо голос ОДРАЗУ (100 мс, як dmrDataKeyTx),
				 * без квитанції -- сам приватний виклик і є відповіддю. */
				s_monReq = requester;
				s_monKeyTries = 0;
				addTimerCallback(monitorKeyVoice, 100, MENU_ANY, false);
				break;
		}
	}
}
